...
namespace Xiidea\EasyGuiBundle\Generator;
...
class DoctrineEntityGenerator extends Generator
{
     ...
     */
    public function generate(BundleInterface $bundle, $entity, $format, array $fields, array $associations)
    {
        ...
        foreach ($associations as $association) {
            if(!is_array($association) || !isset($association['type'])) {
                break;
            }
            switch ($association['type']) {
                case ClassMetadataInfo::ONE_TO_ONE:
                case 'one':
                    $class->mapOneToOne($association);
                    break;
                case 'many':
        ...
        if ('annotation' === $format) {
            ...
            $class->setPrimaryTable(array('name' => Inflector::pluralize(Inflector::tableize(str_replace('\\', '', $entity)))));
